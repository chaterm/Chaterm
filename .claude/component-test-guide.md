...
> Reference: [Vitest Official Component Testing Guide](https://vitest.dev/guide/browser/component-testing.html)
...
test('ProductList filters and displays products correctly', async () => {
  ...
  const { getByLabelText, getByText } = render(<ProductList products={mockProducts} />)
...
test('ContactForm handles complex validation scenarios', async () => {
  ...
  const { getByLabelText, getByText } = render(<ContactForm onSubmit={mockSubmit} />)
