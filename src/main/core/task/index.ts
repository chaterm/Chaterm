...
export class Task {
  ...
  private async resumeTaskFromHistory() {
    ...
    const existingApiConversationHistory: Anthropic.Messages.MessageParam[] =
      await getSavedApiConversationHistory(this.getContext(), this.taskId)
  ...
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const disableBrowserTool =
      vscode.workspace.getConfiguration('cline').get<boolean>('disableBrowserTool') ?? false
    ...
    const globalClineRulesFilePath = await ensureRulesDirectoryExists() // check一下rule到底是什么？
    const globalClineRulesFileInstructions = await getGlobalClineRules(
    ...
    const iterator = stream[Symbol.asyncIterator]() // 迭代器是什么？

    try {
      // awaiting first chunk to see if it will throw an error
      this.isWaitingForFirstChunk = true
      const firstChunk = await iterator.next()
      yield firstChunk.value // yield关键字的作用
            ...
            try {
              ...
              if (block.partial) {
                ...
                if (block.name === 'replace_in_file' && !diff) {
                  this.consecutiveMistakeCount++
                  pushToolResult(
                    await this.sayAndCreateMissingParamError('replace_in_file', 'diff')
                  )
                  await this.diffViewProvider.reset()
                ...
                if (block.name === 'write_to_file' && !content) {
                  this.consecutiveMistakeCount++
                  pushToolResult(
                    await this.sayAndCreateMissingParamError('write_to_file', 'content')
                  )
                  await this.diffViewProvider.reset()
            ...
            try {
              if (block.partial) {
                ...
                const ignoredFileAttemptedToAccess =
                  this.clineIgnoreController.validateCommand(command)
  ...
  async recursivelyMakeClineRequests(
    ...
    const isFirstRequest =
      this.clineMessages.filter((m) => m.say === 'api_req_started').length === 0
    if (isFirstRequest) {
    ...
    // classic/original_autogpt/autogpt/app/configurator.py
    // docs/content/challenges/building_challenges.md
    // classic/forge/forge/components/web/test_search.py
    // classic/forge/forge/components/git_operations/__init__.py
    // classic/original_autogpt/tes…use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)
    // </environment_details>
