...
export class Controller {
  ...
  private async fetchMcpMarketplaceFromApi(
    ...
    try {
      ...
      if (!silent) {
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to fetch MCP marketplace'
        await this.postMessageToWebview({
          type: 'mcpMarketplaceCatalog',
          error: errorMessage
        })
  ...
  private async fetchMcpMarketplace(forceRefresh: boolean = false) {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to handle cached MCP marketplace'
      await this.postMessageToWebview({
        type: 'mcpMarketplaceCatalog',
        error: errorMessage
      })
...
Here is the project's README to help you get started:\n\n${mcpDetails.readmeContent}\n${mcpDetails.llmsInstallationContent}`
  ...
  async getTaskWithId(id: string): Promise<{
    ...
    const history =
      ((await getGlobalState(this.context, 'taskHistory')) as HistoryItem[] | undefined) || []
