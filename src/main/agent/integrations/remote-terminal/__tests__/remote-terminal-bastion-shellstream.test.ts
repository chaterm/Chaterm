...
describe('RemoteTerminalProcess bastion execution', () => {
  ...
  it('rejects when bastion capability lacks getShellStream', async () => {
    ...
    await expect(process.run('session-1', 'whoami', undefined, 'qizhi')).rejects.toThrow(
      /BASTION_AGENT_EXEC_UNAVAILABLE/i
    )
  })
  ...
  it('rejects when bastion capability is missing and does not fall back to SSH', async () => {
    ...
    await expect(process.run('session-1', 'whoami', undefined, 'tencent')).rejects.toThrow(
      /BASTION_CAPABILITY_NOT_FOUND/i
    )
