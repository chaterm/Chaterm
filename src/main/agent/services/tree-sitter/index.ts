...
export async function parseSourceCodeForDefinitionsTopLevel(
  ...
  const allowedFilesToParse = chatermIgnoreController
    ? chatermIgnoreController.filterPaths(filesToParse)
    : filesToParse

  for (const filePath of allowedFilesToParse) {
    const definitions = await parseFile(filePath, languageParsers, chatermIgnoreController)
    if (definitions) {
...
async function parseFile(
  ...
  if (chatermIgnoreController && !chatermIgnoreController.validateAccess(filePath)) {
    return null
  }
