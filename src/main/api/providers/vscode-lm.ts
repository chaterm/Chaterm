...
export class VsCodeLmHandler implements ApiHandler, SingleCompletionHandler {
  ...
  private async calculateTotalInputTokens(
    ...
    return (
      systemTokens + messageTokens.reduce((sum: number, tokens: number): number => sum + tokens, 0)
    )
  }
      ...
      for await (const chunk of response.stream) {
        if (chunk instanceof vscode.LanguageModelTextPart) {
          ...
          if (typeof chunk.value !== 'string') {
            console.warn(
              'Cline <Language Model API>: Invalid text part value received:',
              chunk.value
            )
            continue
          }
          ...
          try {
            ...
            if (!chunk.callId || typeof chunk.callId !== 'string') {
              console.warn(
                'Cline <Language Model API>: Invalid tool callId received:',
                chunk.callId
              )
              continue
            }
  ...
  getModel(): { id: string; info: ModelInfo } {
    if (this.client) {
      ...
      const modelParts = [this.client.vendor, this.client.family, this.client.version].filter(
        Boolean
      )
