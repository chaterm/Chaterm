...
1. ✅ 通过所有检查：`npm run lint && npm run typecheck && npm test`
2. ✅ 未引入无关文件的格式化变更 (检查 git diff)
3. ✅ 提交信息符合 Conventional Commits 格式
4. ✅ 如涉及 UI 变更，已更新双语文案 (zh-CN + en-US)
5. ✅ 如修改数据库，已创建迁移文件
6. ✅ 如新增 IPC 通道，已在 `src/preload/index.d.ts` 定义类型
7. ✅ 未提交敏感信息 (密钥、Token、私有域名)
